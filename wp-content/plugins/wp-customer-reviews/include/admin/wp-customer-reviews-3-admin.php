			...
			foreach ($sectionArr as $settingArr) {
				...
				if ($options->type === "text") {
					...
					foreach ($options->options as $valObj) {
						...
						$default_options[$name][$valObj->value]['label'] = wp_kses($valObj->label, $this->allowedFieldTags);
						foreach ($valObj->checkboxes as $cbObj) {
			...
			if ($options->type === "text") {
				...
				foreach ($options->options as $valObj) {
					...
					if (isset($postVal[$valObj->value]['label'])) {
						$label = wp_kses($postVal[$valObj->value]['label'], $this->allowedFieldTags);
						$default_options[$name][$valObj->value]['label'] = $label;
					}
					foreach ($valObj->checkboxes as $cbObj) {
